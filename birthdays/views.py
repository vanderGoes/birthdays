...
def stats(request):
    ...
    stats = {
        ...
        "Extended records total": Person.objects.annotate(num_sources=Count("sources")).filter(num_sources__gt=0).count(),
        ...
        "Extending source records total": PersonSource.objects.filter(master__isnull=False).count(),
